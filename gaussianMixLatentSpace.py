...
#This class maintains a gaussian mix latent space. there are number of components in the mix. It maintains a mean, std, proportion for each of them. It provides utility to sample points from this mixture too.
...
class GaussianMixLatentSpace(nn.Module):
    def __init__(self, Mu, Cov, Sample2Component, Proportions, Sample_Sizes, points_per_component=100, sigma=0.1):
        ...
        Args:
            ...
            Sample2Component (list): A list of lists, where each inner list contains the indices of the Gaussian
                components that correspond to a sample.
        ...
        self.Mu = nn.ParameterList([nn.Parameter(torch.tensor(mu, dtype=torch.float32), requires_grad=False) for mu in Mu])
    ...
    def comp_posterior(self, si, ci, x):
        return self.GMM.comp_posterior(si, ci, x)
